...
% A library Chart is a type of Helm Chart that defines chart primitives or definitions which can be % shared by Helm templates in other charts. In SKAMPI, many charts are collections of TANGO device % servers so it is possible to harmonize their definition with a library. Fig.~\ref{fig:values_data_model} shows a data model diagram for the harmonized values file.
