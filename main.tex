...
Leaving out the details of the specific components of SKA, fig.~\ref{fig:skampi} shows that all charts depend on some shared charts, like tango-base (refer to section \ref{SKA-docker}) and, optionally, archiver (refer to section \ref{SKA-docker}) and webjive.  
...
\subsection{SKA-docker} \label{SKA-docker}
One of the most important ska repository is the one called \textit{SKA-docker}~\cite{SKA-docker} and contains the definition of a containerized TANGO environment and two helm charts for installing it in k8s: tango-base and webjive. 
